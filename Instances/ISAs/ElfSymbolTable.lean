...
Reads an ELF64 binary to extract function symbols from `.symtab`/`.strtab`.
Pure byte-level parsing, no external dependencies.
...
-- private def eShstrndx : Nat := 62    -- e_shstrndx: section name string table index
...
-- private def stOther : Nat := 5       -- st_other (1 byte, visibility)
...
def readSymbols (data : ByteArray) : Except String (Array (String × UInt64 × UInt64)) := do
  -- Validate ELF magic
  if data.size < 64 then throw "File too small for ELF header"
  for i in [:4] do
