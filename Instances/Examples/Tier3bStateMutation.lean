...
namespace Instances.Examples.Tier3bStateMutation
...
def expectedContinue : Summary Reg :=
  { sub := SymSub.write (SymSub.write SymSub.id .r0 (.add64 (.reg .r0) (.const 1))) .r1 (.const 0)
    pc := .and .true (.not condP) }

def expectedPathTaken : Summary Reg := Summary.compose expectedTaken Summary.id
def expectedPathContinue : Summary Reg := Summary.compose expectedContinue Summary.id
...
end Instances.Examples.Tier3bStateMutation
